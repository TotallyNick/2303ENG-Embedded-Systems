...
char read_UART3(){
		...
		while(UART3->FR & (1<<4)); // wait until bit 6 of the UART FLAG is on that means the FIFO register is Full 
...
int main(void) {
	...
	while(1) {
		...
		c=read_UART3();    // read from Uart (rewrite function for Part 3)
